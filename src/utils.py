...
FOLDER_TIMESTAMP = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d")
# FOLDER_TIMESTAMP = datetime.date.today()
...
def get_game_path(folder: str, game: str, output_path: str) -> str:
    return os.path.join(output_path, "media", get_valid_game_name(game), str(FOLDER_TIMESTAMP), folder)
